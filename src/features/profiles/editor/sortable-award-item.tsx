...
import { GripVertical, EyeIcon, EyeOffIcon } from 'lucide-react';
...
export function SortableAwardItem({ award, onToggleVisibility }: SortableAwardItemProps) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`flex items-center gap-3 rounded-lg border border-gray-800 bg-stone-950 p-2 ${
                !award.isVisible ? 'opacity-50' : ''
            }`}
        >
            <button
                {...attributes}
